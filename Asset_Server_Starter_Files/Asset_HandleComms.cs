...
namespace Asset_ServerPlugin
{
    ...
    [ExportMetadata("Description", "This is the Asset implant handle comms")]
    public class Asset_HandleComms_Base : IExtimplantHandleComms
    {
        private readonly ExtImplantHandleComms_Base extImplantHandleComms_Base = new();

        //overriden as an example since the default performs encryption and decryption and Asset does not need that
        ...
        {
            Console.WriteLine($"{DateTime.UtcNow} handling POST request from Asset");
            byte[] Data;
